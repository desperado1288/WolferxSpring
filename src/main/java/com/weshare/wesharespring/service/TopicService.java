...
public class TopicService {
    ...
    public List<Topic> getAll()
        ...
        try {
            final List<Topic> topicList = topicDao.getAll(Constant.TOPIC_STATUS_ACTIVE);
    ...
    public Topic createTopic(final Topic topic)
        ...
        try {
            ...
            final Long topicId = topicDao.create(topic.getUserId(), topic.getTopicTitle(),
                topic.getTopicDetail(), topic.getTopicTarget(), topic.getTopicLength(),
                topic.getTopicType(), Constant.TOPIC_STATUS_ACTIVE, timeNow);
    ...
    public Topic updateTopic(final Topic topic)
        ...
        try {
            ...
            final int changes = topicDao.update(topic.getTopicId(), topic.getTopicTitle(), topic.getTopicDetail(),
                topic.getTopicTarget(), topic.getTopicLength(), topic.getTopicType(), timeNow);
            if (changes == 0) {
                throw new ItemNotFoundException();
            }
